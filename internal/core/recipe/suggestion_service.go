...
func (s *SuggestionService) SuggestRecipes(ctx context.Context, req *common.RecipeByIngredientsRequest) (*common.RecipeByIngredientsResponse, error) {
	// 驗證必要欄位
	...
	prompt := fmt.Sprintf(`請根據以下可用食材和設備，推薦適合的食譜。
...
請以以下 JSON 格式返回（僅作為範例，請勿直接複製內容）：
{
	...
	common.LogInfo("SuggestRecipes 組裝的 prompt", zap.String("prompt", prompt))
	...
	var result common.RecipeByIngredientsResponse
	if err := common.ParseJSON(content, &result); err != nil {
		return nil, fmt.Errorf("failed to parse AI response: %w", err)
	}

	// 確保每個步驟都有 warnings 欄位
	for i := range result.SuggestedRecipes {
		for j := range result.SuggestedRecipes[i].Recipe {
			if result.SuggestedRecipes[i].Recipe[j].Warnings == nil {
				result.SuggestedRecipes[i].Recipe[j].Warnings = nil
			}
		}
	}
